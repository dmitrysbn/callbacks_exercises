...
var allVendors = purchases.map(purchase => purchase.vendor);
...
var customers = sales.map(sale => sale.customer);
...
var uniqueCustomers = uniques(customers);
...
*/
var bigSales = sales.filter(sale => sale.items.length >= 5);
function nameAndItems(sale) {
  return {
    name: sale.customer,
...
var bigSpenders = bigSales.map(sale => nameAndItems(sale));
...
function sumOfTransaction(transaction) {
...
var sumSales = sales.map(sale => sumOfTransaction(sale)).reduce(sum);
...
var sumPurchases = purchases
  .map(purchase => sumOfTransaction(purchase))
  .reduce(sum);
...
*/
function numOfItems(transaction) {
  return transaction.items.length;
}

var mostItems = transactions
  .map(transaction => numOfItems(transaction))
  .sort(function(a, b) {
    return b - a;
  })[0];
...
*/
var sumOfSmallestPurchase = sumOfTransaction(purchases
  .sort(function(a, b) {
      return numOfItems(a) - numOfItems(b);
    }
  )
  [0]);
